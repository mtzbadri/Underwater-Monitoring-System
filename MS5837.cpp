...
bool MS5837::init(TwoWire &wirePort) {
    ...
    for ( uint8_t i = 0 ; i < 7 ; i++ ) {
        ...
        _i2cPort->requestFrom(static_cast<int>(MS5837_ADDR), 2);
...
void MS5837::read() {
    // Check that _i2cPort is not NULL (i.e., has the user forgotten to call .init or .begin?)
    if (_i2cPort == NULL)
    {
        return;
    }
    ...
    _i2cPort->requestFrom(static_cast<int>(MS5837_ADDR), 3);
    D1_pres = 0;
...
// In order to calculate the correct depth, the actual atmospheric pressure should be measured once in the air, and
